// Zero-dependency fetch wrapper around the UniRate REST API.
//
// Native fetch is required (Node 18.17+ on the build/server side, every
// evergreen browser on the client side). The client is decoupled from
// React on purpose so it stays trivially testable and so consumers
// can also call it imperatively (event handlers, server actions, etc).
//
// `Accept: application/json` is set on every request because UniRate's
// /api/currencies endpoint returns an HTML 404 without it.
...
export class UniRateClient {
  ...
  async listCurrencies(signal?: AbortSignal): Promise<string[]> {
    const raw = await this.#get<{ currencies?: string[] }>("/api/currencies", {}, signal);
  ...
  async getHistoricalRate(
    ...
    }>("/api/historical/rates", params, signal);
